...
void UeasyFM::SimulateControls(float DeltaTime, float alpha, float slip, float pitchrate, float yawrate, float rollrate, float speed, const FTransform &Transform, FVector rawVelocity, float air) {
		...
		if (Guided) {
			...
			if (GuidedToLocation) {
				targetVec = (GuidedVector - Transform.GetLocation()).GetSafeNormal();
			}
			else {
