...
FVector UeasyFM::Control(const FTransform &Transform, FVector velocity, float air) const {
	...
	if (!LinearControlSensitivity) {
		return controltorque*(pow(velocity.Size(), 2.0f))*air;
	}
	else { return controltorque* velocity.Size() *air; };
}
...
FVector UeasyFM::Stability(const FTransform &transform, FVector velocity, float air, float soundv) const {
	...
	//mach stability
	if (MachCurves) {
		float mach = velocity.Size() / soundv;
	...
	if (!LinearStability) {
		return stabtorque * FMath::Pow(velocity.Size(), 2.0f) * air;
	}
	else { return stabtorque * velocity.Size() * air; };
}
