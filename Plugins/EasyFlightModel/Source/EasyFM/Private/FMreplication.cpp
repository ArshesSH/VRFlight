...
void UeasyFM::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	DOREPLIFETIME(UeasyFM, EngineEnabled);
	DOREPLIFETIME(UeasyFM, RCSEnabled);
	...
	DOREPLIFETIME_CONDITION(UeasyFM, ReverserPos, COND_InitialOnly);
...
void UeasyFM::PositionRep_Implementation(FVector NewLocation, FRotator NewRotation, FVector NewVelocity, FVector NewAngularVelocity) {
	...
	if ((GetOwner()->GetLocalRole() != ROLE_Authority) && (Parent != nullptr)) {
...
void UeasyFM::SoftSync(FBodyInstance* BodyInst, FVector NewLocation, FRotator NewRotation, FVector NewVelocity, FVector NewAngularVelocity) {
	...
	//jitter filter
	if (JitterFilter) {
		FVector FilteredLocation = FMath::ClosestPointOnSegment(LocalLoc, NewLocation - (LocalVel * JitterFilterMaxTime), NewLocation + (LocalVel * JitterFilterMaxTime));
